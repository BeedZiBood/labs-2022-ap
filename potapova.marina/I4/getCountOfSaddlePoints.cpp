...
bool isMinInRow(long long* min_elems_in_rows, const long long* const* const matrix, const size_t count_rows,
                const size_t count_cols, const size_t num_row, const long long elem)
{
  if (min_elems_in_rows == nullptr)
  {
    ...
    for (const long long* const* cur_row_ptr = matrix; cur_row_ptr < matrix + count_rows * count_cols;
...
bool isMaxInCol(long long* max_elems_in_cols, const long long* const* const matrix, const size_t count_rows,
                const size_t count_cols, const size_t num_col, const long long elem)
{
...
{
  size_t result = 0;
  long long* min_elems_in_rows = nullptr;
  long long* max_elems_in_cols = nullptr;
